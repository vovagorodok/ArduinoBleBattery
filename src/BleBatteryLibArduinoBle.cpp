...
BLECharacteristic levelStatusCharacteristic(BLE_BATTERY_CHARACTERISTIC_UUID_LEVEL_STATUS, BLERead | BLENotify,
                                            sizeof(BleBatteryLevelStatus));
BLECharacteristic timeStatusCharacteristic(BLE_BATTERY_CHARACTERISTIC_UUID_TIME_STATUS, BLERead | BLENotify,
                                           sizeof(BleBatteryTimeStatus));
BLECharacteristic criticalStatusCharacteristic(BLE_BATTERY_CHARACTERISTIC_UUID_CRITICAL_STATUS, BLERead | BLENotify,
                                               sizeof(BleBatteryCriticalStatus));
}

bool BleBatteryLib::begin(const char* deviceName, BleBatteryService& batteryService) {
    if (!BLE.begin()) {
        return false;
    }
